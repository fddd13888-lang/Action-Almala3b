...
io.on('connection', (socket) => {
  ...
  socket.on('place_card', ({ cardId, zone }) => {
    ...
    // Zone limits
    const zoneLimits = { GK: 1, DEF: 5, MID: 4, ATK: 3 };
    const targetZoneForLimit = card.zone === 'JOKER' ? zone : card.zone;
    if (zoneLimits[targetZoneForLimit] && player.field[targetZoneForLimit] && player.field[targetZoneForLimit].length >= zoneLimits[targetZoneForLimit]) {
      return socket.emit('error', { msg: `المنطقة ممتلئة! الحد الأقصى ${zoneLimits[targetZoneForLimit]}` });
    }
  ...
  socket.on('move_joker', ({ cardId, newZone }) => {
    ...
    // تأكد إن الكارت جوكر وموجود في الملعب
    let foundZone = null, foundIdx = -1;
    for (const z of ['GK','DEF','MID','ATK']) {
      const i = player.field[z].findIndex(c => c.id === cardId && c.joker);
      if (i !== -1) { foundZone = z; foundIdx = i; break; }
    }
    if (!foundZone) return socket.emit('error', { msg: 'الجوكر مش موجود في ملعبك!' });
    if (foundZone === newZone) return socket.emit('error', { msg: 'الجوكر في نفس المنطقة!' });

    // تحقق من حد اللعب (2 لعبة في الدور)
    player._placedThisTurn = (player._placedThisTurn || 0) + 1;
    if (player._placedThisTurn > 2) {
      player._placedThisTurn--;
      return socket.emit('error', { msg: 'وصلت الحد الأقصى (2 لعبة في الدور)!' });
    }

    // نقل الجوكر
    const [joker] = player.field[foundZone].splice(foundIdx, 1);
