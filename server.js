...
io.on('connection', (socket) => {
  ...
  socket.on('place_card', ({ cardId, zone }) => {
    ...
    // First turn limit: max 2 cards
    if (room.firstTurn) {
      const placed = player.hand.length; // will check placed count via event
      // We track placed count per turn via a temp counter
      player._placedThisTurn = (player._placedThisTurn || 0) + 1;
      if (player._placedThisTurn > 2) {
        player._placedThisTurn--;
        return socket.emit('error', { msg: 'الدور الأول: ممكن تحط 2 كروت بس!' });
      }
    }
  ...
  socket.on('end_turn', () => {
    ...
    // Enforce hand limit
    enforceHandLimit(nextPlayer, room.deck);
...
server.listen(PORT, () => console.log(`Action Almala3b Server running on port ${PORT}`));
